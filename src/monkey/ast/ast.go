...
type BlockStatement struct {
	Token      token.Token // Will be the { token
	Statements []Statement
}
...
type IfExpression struct {
	Token       token.Token // 'if' Token here
...
type CallExpression struct {
	...
	Function  Expression  // Must be Identifier (pointing to a FunctionLiteral, but that's not checked here) or FunctionLiteral
...
type ArrayLiteral struct {
	Token    token.Token // Here, the '[' token indicating start
	Elements []Expression
}
...
// Expressions representing indexing into array literals.
...
type HashLiteral struct {
	Token token.Token               // Here, the '{' token
	Pairs map[Expression]Expression // The key and value can be any expressions, provided the key evaluates to a string, boolean or integer
}
